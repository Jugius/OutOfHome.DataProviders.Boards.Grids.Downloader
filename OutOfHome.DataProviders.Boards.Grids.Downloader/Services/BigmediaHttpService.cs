...
public class BigmediaHttpService
{
    ...
    private readonly HttpEngine<GetGridRequest, List<BmaBoard>, ContentParser> _getGridEngine;
    private readonly HttpEngine<AuthenticateRequest> _authenticateEngine;

    public BigmediaHttpService(HttpClient httpClient)
    {
        _httpClient = httpClient;
        _getGridEngine = new HttpEngine<GetGridRequest, List<BmaBoard>, ContentParser>(_httpClient);
    ...
    public async Task<List<BmaBoard>> GetGrid(GetGridRequest request) => await _getGridEngine.QueryAsync(request);
